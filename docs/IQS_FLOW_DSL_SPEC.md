...
> **断言口径**: `npm run test:flow` → parser 66 + svg 85 + bpmn 17 + mainline 8 + cell_order 8 = **184**
...
- 岗位通过属性 `Role(<值>)` 标注在节点上（见 §6.2）。岗位不参与格子坐标（岗位不占格）。
- **节点右下角**：按 `Attr active` 顺序，只渲染该节点**有值**的项（字典引用展开），多行右对齐，至多 4 行。未写 `Attr active` 时回退只标 Role（兼容旧图）。空键（如未写 Lv）不占行。
...
6. **岗位图例栏**：从**节点实际出现的 `Role` 属性值**去重提取，按**首次出现顺序**排列、按**出现次数**定字体粗细（非渲染 R 数组本身）。节点右下角按 `Attr active` 顺序显示该节点有值项（无 active 时仅 Role）。
...
11. **连线交叉**：正交真交点（端点 T 接不算）中，**线序更大**（后声明/后绘制）的整条连线改用 `contrastStroke(Color[Line], Color[Panel])`（连线色与底色的公共差异色），压在线序较小的本色连线之上，以区分两条线。
