...
| R21 | 2026-09-12 | **使用者实测驱动**：渲染缓慢（P0）· 子流程不继承布局 · 连线缺目标 | 3 | 138–140 |
| R22 | 2026-09-12 | **R21 复测驱动**：复核 R21 三项修复（1 正确 / 1 不完整 / 1 机制判错）· 端口朝向 + 回折穿盒 + 子流程整数倍格 · 新增几何门禁 G1–G5 + 注入回归 · 断言口径勘误 | 6 | 141–146 |
| R23 | 2026-09-12 | **出入口红线 + 子流程范式**：同入/同出可复用、同侧既入又出为硬约束；R22 `relaxNodes` 误把 A1 对网关关闭；子流程内部仍是缩略图而非 3×3 绘制格 | 5 | 147–151 |
| R24 | 2026-09-13 | **口径补全 + 代数下降性能 + 子流程画布同底**：N/DATA 水平右入、扩展格射线、A1 前瞻、共干非槽位；端口下降改为代数核+缓存；子流程框/标题/溢出跟面板色 | 3 | 152–154 |
...
| 优先级 | 条目 | 动作 |
## R21 — 使用者实测驱动（2026-09-12）

> **触发**：使用者在画布上实测，提出三个问题并附截图：
> ① 子流程未继承外层 `Layout`；② 出现不指向可见目标的连线；③ 渲染明显缓慢。
> **方法**：**先查全部工程记录**（`docs/flow/{notes,review,design}` 共 38 份）划定已知边界，再逐层定位到代码行。
> **完整报告**：`docs/flow/review/FLOW_COMPONENT_AUDIT_R21.md`

### 与既有记录的关系（先划边界，避免重复劳动）

| 事项 | 记录状态 | 本轮 |
|:--|:--|:--|
| 子流程**内嵌小图** | ✅ 已实现（`FLOW_LAYOUT_NOTES_STAGES1-3.md` §4 阶段三b，svg 断言 45→51） | 确认 |
| 子流程**退出主网格** | ✅ 已实现（`FLOW_LAYOUT_NOTES_ALIGN_RECOVERY.md`，2026-08-31） | 确认 |
| `SUB` 打断默认序流须显式连 | ✅ 已记录已修（AUD-119/122） | 确认 |
| 默认顺序流**只跳过网关** | ✅ 已记录（`FLOW_LAYOUT_NOTES_RECOVERY_P0.md:94`） | 与 AUD-139 相关 |
| 端口优化**单图 ~0.3s** | ✅ 已自记（`FLOW_ROUTING_WORKLOG.md:643`「0.3s 量级（可接受）」） | **该判断需修正，见 AUD-140** |
| 子流程**继承 Layout** | ❌ 全部记录查无 | **新发现** |
| `FlowDiagram` **渲染缓存** | ❌ 查无 | **新发现** |

### 条目

| 编号 | 级别 | 状态 | 主题 | 结论 |
|:--|:--:|:--|:--|:--|
| **AUD-138** | P1 | **已修复** | 子流程不继承外层 `Layout` | `flowToSVG.ts` 的 `renderSubprocessInner` 内部列数原为固定 `Math.ceil(Math.sqrt(n))` —— 函数**收了 `data` 却从不读 `data.layout`**，故 `Layout: H` 时子流程内部也不横排。另迷你字号 `fs=8` 低于范式下限 11。**数据层无缺陷**：探针实测 `subProcesses:[{id:"sub1",nodes:["s1","s2"]}]`、`s1/s2` 的 `parent="sub1"`、内部边 `["s1","s2"]` 均在位，`errors:[]` —— 缺陷纯在渲染层。**修复**：列数改由 `data.layout` 决定（`H`→一行 / `V`→一列）并以可用宽度封顶；`fs` 8→10 |
| **AUD-139** | P1 | **已修复** | 画布出现不指向可见目标的连线 | 默认顺序流的**源侧跳过集合只含网关与 `end`/`N`/`DATA`**，未列 `subprocess` —— 与 **AUD-119 同源机制**。当前因 `a.parent !== b.parent` 多数情况已被拦住，但语义上必须显式声明。**附带查出示例自身缺陷**：canonical 示例中 `p1`（并行网关）与 `w3`（任务）**共用 `worker[3]`** → 画布出现两个同名「物理指标检测」节点，是「指向不明」观感的另一半来源。**修复**：源侧补入 `subprocess`；示例字典插入「物理/化学并行检测」并拆分索引 |
| **AUD-140** | **P0** | **已修复** | 渲染缓慢（交互可用性） | `FlowDiagram.tsx` 的 `const svg = flowToSVG(data, finalStyles)` 为**裸调用**（全组件 178 行**零 `useMemo`**），且 `finalStyles` 每次 render 新建对象；同时 `dangerouslySetInnerHTML` **每次 render 重新解析并写入整段 SVG**。拖拽画布 → `setView` → **重算 `flowToSVG`（含布局 + 0.3s 端口路由）+ 全量 DOM 重写**，而 `data`/`styles` 未变。`FLOW_ROUTING_WORKLOG.md:643` 判「0.3s 可接受」的前提是**一次生成**，进入交互循环后不成立。**修复**：四项 `useMemo` 化；`dangerouslySetInnerHTML` → `<g ref>` 仅随 `innerSvg` 单次写入 → 拖拽/缩放每帧只剩一个 CSS transform |

> ⚠️ **R22 补注（2026-09-12）**：本节三项中 **AUD-140 修复正确**；
> **AUD-138 修复不完整**（只改渲染层 `renderSubprocessInner`，布局层 `ExcelLayout` 仍用 `√N` 方阵 → 两层数学分叉，现象未消）；
> **AUD-139 归因不成立**（对 canonical 示例，默认顺序流并未产生 `sub1` 的隐式出边 —— 实测 13 条显式/分支边 + 1 条默认边 `w1→w2`；
> 真因是**端口背向** + **路径回折穿源节点自身盒**）。已由 R22 补完（`AUD-141..146`），
> 详见 `docs/flow/review/FLOW_COMPONENT_AUDIT_R22.md`。本段历史原文保持不覆盖。

### 范式增量（跨轮通用）

- **门禁须自证有效**：任何新增校验门禁，**必须**做一次注入回归（故意造出应拦截的错误，确认报红，再恢复）。否则无法区分「真的通过」与「根本没跑」。
- **门禁语义须自洽**：「产物是否新鲜」不得用 `git diff --exit-code`（产物常处未提交态，会恒红）；应比对**重生成前后的哈希**。
- **渲染层的 memo 边界**：凡「上游只依赖 `data`/`styles`」的昂贵计算（布局、路由、SVG 序列化），必须与「交互态」（`view` 的 pan/zoom）**解耦** —— 后者只能改 transform，不得触发前者。
...
npm run test:flow  EXIT=0  191 pass / 0 fail（6 段）
npm run validate:dsl   EXIT=0  3 段 PASS
npm run validate:cards EXIT=0  PASS (42 warnings)
npm run check:cards-fresh EXIT=0
```

**提交**：`2535978 fix(flow): 渲染缓存、子流程继承布局、默认流跳过集合（AUD-138/139/140）`
...
## R22 — R21 复测驱动（2026-09-12）

> **触发**：使用者对 R21 修复结果复测，重提同一两项（连线缺目标 / 子流程未按范式显示，附截图）。
> **方法**：先查全部 FLOW 工程记录划定判据来源，再用探针打出**渲染实际使用的那条路径**，定位到函数与行。
> **完整报告**：`docs/flow/review/FLOW_COMPONENT_AUDIT_R22.md`

### 对 R21 的复核结论

| R21 条目 | 复核 |
|:--|:--|
| AUD-140 渲染缓存 | ✅ **修复正确**（memo 边界 + DOM 写入与视图变换解耦） |
| AUD-138 子流程继承布局 | ⚠️ **修复不完整** —— 只改渲染层，布局层 `ExcelLayout` 仍 `√N` 方阵 ⇒ 两层数学分叉 |
| AUD-139 连线缺目标 | ❌ **归因不成立** —— 默认顺序流并非成因（实测 13 显式/分支边 + 1 默认边），真因在端口朝向与路径几何 |

### 条目

| 编号 | 级别 | 状态 | 主题 | 结论 |
|:--|:--:|:--|:--|:--|
| **AUD-141** | P1 | **已修复** | 路径**穿源/目标自身盒** | 两内核均写有 `if (id === from.id \|\| id === to.id) continue;` —— **整体豁免自身盒**，使「绕出去再折回、穿过自己盒子」的畸形路径凭折弯更少胜出（实测 `w2→g1` 横穿 w2、`p1→w4` 纵穿 p1）。**修复**：自身盒**仍须检测**，判定盒收缩 3px（`self ? -3 : 4`），stub 段自边界出发自然豁免 |
| **AUD-142** | P1 | **已修复** | `countBends` **漏计 180° 回折** | `computeCost` 在 R19 已补符号项，而 `countBends`（端口评估 + 混合内核择优共用）未补 ⇒ 回折路被系统性低估。**修复**：补 `else if (d1x*d2x + d1y*d2y < 0) b += 2;`，与 `computeCost` 同口径（R21 所记 P0 余项 `083/107` 闭环） |
| **AUD-143** | P1 | **已修复** | 目标端口**背向**（箭头反指） | 代价函数只看折弯数，0 折弯的背向解（`d1: L → p1: L`，源在右侧却从左侧进入）压过方向正确的解 ⇒ 箭头尖端背离来向，观感"线没落到目标上"。**修复**：`costOf = 折弯 + 1000 × 背向数`（软惩罚，可降级）；并把 A1 对**网关**放宽为白名单（范式 §1.2/§5.1：汇聚/发散允许复用），否则「源与出边同侧」的图形必被逼成背向 |
| **AUD-144** | P1 | **已修复** | 严格判定下的**缺线**风险 | 收紧碰撞判定后，个别边可能全候选无解 ⇒ 路径为空 ⇒ 边**消失**（实测 `边路径数 5 ≠ 6`）。**修复**：A4（无碰撞）按范式确认为**软约束**（AUD-027），新增 `solveRouteHybrid` 两级：严格档优先，不可达时 `selfBoxStrict:false` 放宽兜底 —— **宁可穿盒，不可缺线** |
| **AUD-145** | P2 | **已修复** | 混合内核**编排重复**（口径分叉） | `flowToSVG`（渲染）与 `PortOptimizer.bendsOf`（端口评估）各自复制「调两内核 + `pickShorter`」⇒ 一旦分叉，端口将按与渲染不同的代价决策。**修复**：收敛为 `VisibleGraphRouter.solveRouteHybrid` **单一真源** |
| **AUD-146** | P2 | **已修复** | 三项**断言口径缺陷** | ① 边路径按 `stroke="#64748b"` 计数 ⇒ 被判为交叉反差的边漏计（本轮成假红）→ 渲染层加语义锚点 `data-edge="1"`；② `cross: 差异色不是连线本色` 绑死色值 → 改从实际 `data-flow="cross-over"` 元素取色比对；③ `a1-wsad` 只校验贪心初值 `solveAlgebraicPorts`，与实际使用的 `optimizePorts` 脱节 → 新增 `g5-a1-whitelist` 校验实际端口 |

### 新增门禁（`scripts/assert_flow_geometry.ts`，12 断言 / 第 7 段）

G1 零穿盒（含自身盒）· G2 零 180° 回折（独立几何判据 + `countBends` 口径自洽）· G3 端口无背向 ·
G4 子流程整数倍格（框 `cols×140 × rows×48` / `H→一行 V→一列` / 迷你字号 ≥ 11 / 空子流程不放大）· G5 A1 白名单外互斥。

**注入回归（门禁自证）**：A 碰撞检测失效 → 报红 `g1-zero-pierce`；B 回退 R21 旧配置 → 报红 `g1-zero-pierce` + `g3-port-facing`；
C 子流程 `√N` 方阵 → 报红 `g4-*`（均注入后还原并校验哈希一致）。
**自查修正**：G2 最初借用 `countBends` 自身做判据 ⇒ 「删回折项」注入下断言恒真，已改为独立几何判据。

### 本轮验证

```
npx tsc --noEmit          EXIT=0
npm run test:flow         EXIT=0  203 pass / 0 fail（7 段；parser 66 / svg 85 / bpmn 17 / mainline 8 / cell_order 8 / alignment 7 / geometry 12）
npm run validate:dsl      EXIT=0  3 段 PASS
...
- **判据不得借用被测实现自身**：门禁若用被测函数（如 `countBends`）验证被测行为，删掉被测逻辑会让断言恒真 ⇒ 门禁形同虚设。判据应**独立几何化**，或另设「口径自洽」断言。
- **断言不得绑定表现层属性**：颜色/字号/文案随主题与交互变化；计数与结构断言应绑定**语义锚点**（如 `data-edge`）。
- **定位层级要落到「现象所在层」**：R21 把渲染缺陷定位到解析层（默认流跳过集合），导致修复无效。现象在画布上 ⇒ 先复算**渲染所用的路径**。
- **单一真源同样适用于编排**：同一段「调 A 再调 B 取优」的两处复制，会使评估口径与渲染口径分叉（R22 的 `solveRouteHybrid`）。

### 未决（交接）

① A1 硬约束 vs 网关白名单（若回硬约束，需先给出「网关同侧汇聚」的可解构造）；② G2 不覆盖「非共线大回环」；
③ 兜底档触发时**无告警**（建议加 `data-flow="route-fallback"` 标记 + lint warn）；④ AUD-061/062/063（M4/M5）仍未按文档实现；
⑤ `getTheoreticalBends` 仍是不看符号的粗分档，为下一处可审计点。

> **R23 补注（2026-09-12）**：未决①已由使用者当场裁决 —— **同入/同出可复用；同侧既入又出是红线，网关不豁免**。
> 「网关同侧汇聚」的可解构造即贪心初值已给出的同向复用（canonical `p1 IN=B×2 OUT=R×2`），不必用放松 A1 换 0 折弯。
> 详见 `docs/flow/review/FLOW_COMPONENT_AUDIT_R23.md`。

---

## R23 — 出入口红线 + 子流程范式（2026-09-12）

> **触发**：LIVE ENGINE 截图（canonical 卷烟批次质量追溯）上并行网关「物理/化学并行检测」同一侧既入又出；子流程「技术中心复核」内部不像外层流程。
> **使用者红线**：**同出可复用、同入可复用；有入有出，不能复用。**
> **完整报告**：`docs/flow/review/FLOW_COMPONENT_AUDIT_R23.md`
...
|:--|:--|
| AUD-141 自身盒穿盒 | ✅ 修复仍成立 |
| AUD-142 `countBends` 回折 | ✅ 修复仍成立 |
| AUD-143 背向罚 + 网关白名单 | ⚠️ **白名单误读** —— 「除非汇聚/发散允许复用」修饰的是「每端口至多一条边」（A3），不是 A1。`relaxNodes` 把网关的 `in ∩ out = ∅` 关掉，canonical `p1` 在 B、R 两侧既入又出 |
| AUD-144 A4 软兜底 | ✅ 与本轮无关 |
| AUD-145 `solveRouteHybrid` 单一真源 | ✅ 仍成立 |
| G5 跳过网关 | ❌ 把红线违规对 CI 隐藏 |

### 条目

| 编号 | 级别 | 状态 | 主题 | 结论 |
|:--|:--:|:--|:--|:--|
| **AUD-147** | **P0** | **已修复** | 同侧既入又出（出入口同源，**截图上至少两处硬口**） | 按「侧」计而不是按节点收成一处：① `p1` **右边** `(453,267)` = `d1→p1` 虚线入 + `p1→w3` 物理检出；② `p1` **底边** `(391,294)` = `g1→p1` 合规入 + `p1→w4` 化学检出。另：两菱形间竖线「合规」与「化学检测」共线搭乘 186px；`w2→g1` 竖线再贴 p1 右侧，观感上右边三线并发。贪心初值 `p1 IN=[B,B] OUT=[R,R]` **两侧都守住红线**；`optimizePorts`+`relaxNodes` 拆成既入又出。`g1` 本身 `IN={R} OUT={T,B}` **不是** A1 违规。`w8` 左 `L×2` 是合法同入 |
| **AUD-148** | **P0** | **已修复** | G5 门禁跳过网关 | `assert_flow_geometry.ts` 对 `exclusiveGateway`/`parallelGateway` `continue`；唯一违规点 `p1` 对 CI 不可见。`a1-wsad` 仍只测贪心初值。红线无回归保护 |
| **AUD-149** | P1 | **已修复** | 子流程内部不是 3×3 范式 | `subprocessInnerGrid` 只统一框的格数；`renderSubprocessInner` 无 `half` 走廊、无 WSAD、中心穿心连线、迷你尺寸与 `NODE_BASE` 无关。前提②「等比例缩放的流程节点」未落地。内部节点 `cell=null`、不进 `nodePos` |
| **AUD-150** | P1 | **已修复** | 子流程标题侵入 + 内部连线退化 | 标题钉左下角与左格「受理复核」抢 48px 高；同行内部边模板产生 `M x,y L x,y L x2,y L x2,y` 零长 stub，水平段穿过迷你节点中心 |
| **AUD-151** | P2 | **已修复** | 内部铺排误用文档级 Layout | `data.layout !== 'V'` → 横排。文档 `Layout: H` 表示 D 为行、P 为列（AUD-127），不是「孩子从左到右」。内图应走 `CellOrder` / 内边拓扑 |

### 建议摘要

1. A1 对**所有**节点硬 skip（含网关）；`relaxNodes` 最多放开「同侧多条同向边」。
2. 代价优先级：A1 硬 ≫ 背向软罚 ≫ 折弯。禁止用 0 折弯买掉 A1。
3. G5 改为全节点 `in ∩ out = ∅`；用当前 canonical 的 `p1` 同侧入出做注入必红。
4. 子流程框内嵌同一套绘制格几何 + 端口布线；标题独立成带；停止维护第二套迷你折线。

---

---

## R24 — 口径补全与代数下降性能（2026-09-13）

> **触发**：LIVE 上 N/DATA 虚线打折、中间走廊假合并、打开示例耗时数秒。
> **设计**：`docs/flow/design/FLOW_ROUTING_EXCLUSIVITY_DESIGN.md` §5–6 · `FLOW_NDATA_LANE_DESIGN.md` §3.1.4

| 编号 | 级别 | 状态 | 主题 | 结论 |
|:--|:--:|:--|:--|:--|
| **AUD-152** | P1 | **已修复** | 不同终点边共走廊假合并 | 合规占 p1 底入，化学只能左出再走 y=711，与合规横线叠 101px。贪心/下降增加「抢占对角色朝向侧」势能 80（A1 前瞻）。合规改左入，物理/化学同出底边共干 |
| **AUD-153** | P2 | **已修复** | 打开示例端口下降 ~5.5s | `optimizePorts` 对每边 16 对端口调用 `solveRouteHybrid`（含 Dijkstra）。下降改为 `solveAlgebraicRoute` + 缓存；最终渲染代数 ≤2 弯则跳过可见图。canonical `flowToSVG` ~150ms |
| **AUD-154** | P3 | **已修复** | 子流程实心底与内部色块难辨 | 外框 `fill=panelColor`，描边/＋号 `subprocessColor`；标题 `textColor`；超宽与内部溢出衬底用面板色，不再深底白字 |

门禁：`g7-doc-horizontal`；断言总数 **206**。
