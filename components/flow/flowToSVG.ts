/**
 * IQS-Flow 自研 SVG 渲染器（布局算法 v6 + 代数路由）
 * ① 流程绘制格=3×3九小格(中心1W×1H节点+四周8格0.5连线区)
 * ② 泳道交叉格 X×Y 切分(V 纵 / H 横 / D 对角；nx/ny 整行整列统一扩展)
 * ③ 行列统一切分：行高=该行最大纵向堆叠、列宽=该列最大横向并排
 * ④ W列最大宽/H行最大高；节点严格居中绘制格
 * ⑤ 接触点=边中点（贴边）；法向 stub 外延 half 到走廊中线（公理 A2）
 * ⑥ 代数路由：solveAlgebraicPorts（WSAD）+ solveAlgebraicRoute（直连→L→Z→外侧）
 *    旧「3 轮确定性避障」已删除；T2 守护位移 / J* 合并未实施
 * ⑦ 全页面绘制区(fitView一页展现)；N/DATA 落 DOC 虚拟列
 */
